...
/*   Updated: 2021/03/18 17:31:08 by melisha          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		fn_insert_err(t_obj *obj)
{
	...
	if (obj->pars.command[i] == '$' && obj->pars.command[i + 1] == '?')
	{
		...
		!(obj->pars.command = ft_substr(save, 0, i)))
			fn_error("not memory allocate");
		if (!(num = ft_itoa(errno)))
			fn_error("not memory allocate");
...
int				utils_valid_command_cont(t_obj *obj)
{
...
int				utils_valid_command(t_obj *obj)
{
	...
	(obj->pars.command[0] == '/' && obj->pars.command[1] == 'b' &&
	obj->pars.command[2] == 'i' && obj->pars.command[3] == 'n' &&
	obj->pars.command[4] == '/' && obj->pars.command[5] == 'p' &&
	obj->pars.command[6] == 'w' &&
	...
	obj->pars.command[6] == '\0' && (obj->flag.c_flag.exp = 1) != -1)
		return (1);
	return (utils_valid_command_cont(obj));
}
...
int				fn_valid_command(t_obj *obj)
{
	...
	obj->pars.command[4] == '\0') || (obj->pars.command[0] == '/'
	&& obj->pars.command[1] == 'b' &&
	obj->pars.command[2] == 'i' && obj->pars.command[3] == 'n' &&
	obj->pars.command[4] == '/' && obj->pars.command[5] == 'e' &&
	obj->pars.command[6] == 'c' &&
